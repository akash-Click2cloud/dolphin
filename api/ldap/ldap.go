...
const (
	...
	ErrUserNotFound = dockm.Error("User not found or too many entries returned")
)

// Service represents a service used to authenticate users against a LDAP/AD.
type Service struct{}

func searchUser(username string, conn *ldap.Conn, settings []dockm.LDAPSearchSettings) (string, error) {
	var userDN string
	found := false
...
func (*Service) AuthenticateUser(username, password string, settings *dockm.LDAPSettings) error {
...
func (*Service) TestConnectivity(settings *dockm.LDAPSettings) error {
