...
module.exports = function (grunt) {
    ...
    grunt.initConfig({
        ...
        src: {
            js: ['app/**/*.js', '!app/**/*.spec.js','assets/js/*.js'],
            jsTpl: ['<%= distdir %>/templates/**/*.js'],
            html: ['index.html'],
            tpl: ['app/components/**/*.html', 'app/directives/**/*.html'],
            css: ['assets/css/*.css']
            /*css: ['assets/css/app.css','assets/css/animate.min.css','assets/css/base.css','assets/css/font_awesome_4.5.0_min.css','assets/css/style.css']*/
        },
        clean: {
            all: ['<%= distdir %>/*'],
            app: ['<%= distdir %>/*', '!<%= distdir %>/dockm*','!<%= distdir %>/main*'],
        ...
        copy: {
            bundle: {
                files: [
                    ...
                    {dest:'<%= distdir %>/css/', src: ['app.css','animate.min.css','base.css','style.css','font_awesome_4.5.0_min.css'], expand: true, cwd: '.tmp/concat/css/' }*/
                    {dest:'<%= distdir %>/js/',  src: ['*.js'],  expand: true, cwd: '.tmp/concat/js/' },
                    {dest:'<%= distdir %>/css/', src: ['*.css'], expand: true, cwd: '.tmp/concat/css/' }
                ]
            },
            assets: {
                files: [
        ...
        shell: {
            ...
            run: {
                command: [
                    'docker rm -f dockm-go-new',
                    'docker run -d -p 9020:9000 -v $(pwd)/dist:/app -v $(pwd)/userdata:/data -v /var/run/docker.sock:/var/run/docker.sock:z --name dockm-go-new portainer/base /app/dockm-linux-amd64 --no-analytics -a /app'
                ].join(';')
            }
        },
        replace: {
